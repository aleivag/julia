...
""", encoding="utf-8")
    ...
    def test_builds_static_site(self) -> None:
        with TemporaryDirectory() as directory:
            ...
            self.assertIn('const familiarVolumeUnits = new Set(["tsp", "teaspoon", "teaspoons", "tbsp", "tablespoon", "tablespoons", "cup", "cups"]);', app_js)
            self.assertIn('familiarVolumeUnits.has(normalizedUnit(unit))', app_js)
            self.assertIn('return `${original} (${converted})`;', app_js)
            ...
            self.assertTrue((output / "recipes" / "index.html").exists())
            ...
            self.assertIn('class="recipe-search-home" data-search-first', index_html)
            self.assertIn('placeholder="Search recipes, ingredients, or #tags"', index_html)
            self.assertIn('class="search-suggestions" id="recipe-search-suggestions" data-search-suggestions hidden', index_html)
            self.assertIn('class="recipe-search-results" id="recipe-search-results" data-search-results hidden', index_html)
            self.assertIn("recipes ready to search", index_html)
            self.assertRegex(index_html, r'data-search="[^"]*prime rib roast[^"]*"')
            ...
            self.assertIn('href="recipes/index.html">All Recipes Index</a>', index_html)
            recipe_index = (output / "recipes" / "index.html").read_text(encoding="utf-8")
            self.assertIn('data-page="recipe-index"', recipe_index)
            self.assertIn('id="by-name"', recipe_index)
            self.assertIn('id="by-family"', recipe_index)
            self.assertIn('id="by-tag"', recipe_index)
            self.assertIn('id="by-main-ingredient"', recipe_index)
            self.assertIn('data-index-filter', recipe_index)
            self.assertIn('placeholder="Filter by recipe, family, ingredient, or #tag"', recipe_index)
            self.assertRegex(recipe_index, r'data-index-tags="[^"]*beef[^"]*"')
            self.assertIn('href="indoor-smoke-ribs.html">Indoor Smoke Barbecue Ribs</a>', recipe_index)
            self.assertIn("No family", recipe_index)
